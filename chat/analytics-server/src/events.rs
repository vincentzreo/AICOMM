use crate::{
    ...
    pb::analytics::{
        ...
        AppStartEvent, ChatCreatedEvent, ChatJoinedEvent, ChatLeftEvent, EventContext,
        MessageSentEvent, NavigationEvent, UserLoginEvent, UserLogoutEvent, UserRegisterEvent,
    },
    AppError,
};
...
impl TryFrom<AnalyticsEvent> for AnalyticsEventRow {
    ...
    fn try_from(event: AnalyticsEvent) -> Result<Self, Self::Error> {
        ...
        match event.context {
            ...
            None => return Err(AppError::MissEventContext),
        }
        match event.event_type {
            Some(event) => event.consume(&mut ret)?,
            None => return Err(AppError::MissEventData),
        }
        Ok(ret)
    }
}
